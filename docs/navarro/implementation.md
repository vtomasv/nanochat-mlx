...
`native.cpp` constructs CSRV symbols 1+value_id*columns+column, zero delimiter. Exact FP32 bits identify dictionary values. Rules never include delimiters. A deterministic maximum-frequency pair is substituted until none repeats. Frequency ties use ascending pair order. The original campaign used repeated frequency scans; its measurements remain tied to the archived implementation. The hybrid follow-up below replaces those scans without changing pair selection or the format.
...
## Hybrid RePair follow-up

The current independent constructor combines the existing packed format with live linked positions, occurrence lists and a lazy maximum-frequency heap. Replacing a pair updates its neighboring edges instead of rescanning the full sequence. Heap ties retain ascending symbol-pair order; replacements retain left-to-right order. Initial singleton pairs need no occurrence lists because newly created pairs contain the new rule ID. No upstream source was copied and no linear-time complexity claim is made for this implementation.

Adaptive encoding stops when a proven size lower bound exceeds 95% of the dense block. With d distinct nonzero values in N cells, at least max(0, 2d-N) values occur once; their terminals cannot participate in repeated pairs and must remain in the final stream. The bound includes the 64-byte header, immutable FP32 dictionary, and those mandatory terminals at the minimum format width. A binary search computes the distinct-value cutoff once per block. It is not a sampled entropy heuristic and cannot reject a block that the existing format would accept. Forced re_32/re_iv remain available; IEEE special values, negative zero, arithmetic, block sizes, serialization and the final adaptive threshold retain their original behavior.

The [hybrid report](hybrid-repair.md) separates this implementation's tests and measurements from the campaign's original negative verdicts. Baseline comparisons compile our own codec from commit `7787ae2` using identical flags. The code snapshots and raw follow-up evidence live under `experiments/navarro/verification/hybrid-repair-v2/`.
