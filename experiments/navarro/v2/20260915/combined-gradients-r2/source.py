...
def selftest(candidate):
    ...
    for k in range(10):
        ...
        for label,a,b in [('weights',native.parameters(),model.parameters()),('optimizer',{'adam':oa.adam_state,'muon':oa.muon_state},{'adam':ob.adam_state,'muon':ob.muon_state})]:
...
def worker(root,trial_id):
    ...
    trial=root/trial_id;request=json.loads((trial/'request.json').read_text())
    if request['plan_sha256']!=digest(root/'plan.json'):raise ValueError('Registered plan changed')
    arm=request['scenario'];c=m['config']
    ...
    import hashlib
    fingerprints={}
    for group,tree in [('model',model.parameters()),('adam',opt.adam_state),('muon',opt.muon_state)]:
        fingerprints[group]={name:hashlib.sha256(np.asarray(v).tobytes()).hexdigest() for name,v in tree_flatten(tree)}
    write_json(trial/'final-state-fingerprints.json',fingerprints)
    ...
    result={'status':'MEASURED_EXPLORATORY','correctness_pass':True,'scenario':arm,'steps':100,'start_step':100,
        'metrics':metrics,'quality':{k:v for k,v in quality.items() if k!='rows'},'training_seconds':training_seconds,
...
def main():
    parser=argparse.ArgumentParser(__doc__);parser.add_argument('--worker');parser.add_argument('--root',default='experiments/navarro/v2/20260915/combined-gradients-r2');args=parser.parse_args();root=Path(args.root).resolve()
